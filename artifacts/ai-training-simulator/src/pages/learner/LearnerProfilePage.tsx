...
import {
  ...
  RefreshCw, AlertCircle, CheckCircle2, Eye, EyeOff, LogOut, GraduationCap,
} from "lucide-react";
...
export default function LearnerProfilePage() {
  ...
  const displayName    = profile.name  ?? clerkUser?.fullName ?? "Learner";
    ...
    <LearnerLayout>
      ...
      <div className="mb-8 flex items-center gap-5">
        ...
        <div className="min-w-0">
          <h1 className="text-2xl font-display font-bold text-foreground leading-tight truncate">{displayName}</h1>
          {profile.username && (
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-primary/10 text-primary capitalize ring-1 ring-primary/20">
              <GraduationCap className="w-3 h-3" />{profile.role ?? "learner"}
            </span>
      ...
      <div className="max-w-xl space-y-4">
        ...
        {/* Profile */}
        <Section icon={User} title="Profile" description="Your display name and public username." onSave={saveProfile} saveState={profileSave}>
        ...
        {/* Email */}
        <Section icon={Mail} title="Email address" description="Used for sign-in and notifications." onSave={saveEmail} saveState={emailSave}>
          <div className="mt-3">
        ...
        {isLocalAccount && (
          <Section icon={Lock} title="Password" description="Change the password you use to sign in." onSave={savePassword} saveState={pwSave}>
            <div className="space-y-4 mt-3">
        ...
        {/* Notifications */}
        <Section icon={Bell} title="Notifications" description="Control how we contact you." onSave={saveNotifs} saveState={notifSave}>
          <div className="mt-3">
            <div className="flex items-center justify-between gap-6 rounded-lg border border-border/60 bg-muted/30 px-4 py-3">
              <div>
                <p className="text-sm font-medium">Email notifications</p>
                <p className="text-xs text-muted-foreground mt-0.5">Get emailed when new modules are assigned or results are ready.</p>
              </div>
        ...
        {/* Account info */}
        <Section icon={ShieldCheck} title="Account" description="Read-only details about your account.">
